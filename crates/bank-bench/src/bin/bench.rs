...
struct Args {
    ...
    accounts: i64,
    operations: u64,
    /// The engine commit this run is being compared against, when it is one arm of an A/B.
    ///
    /// Recorded, never executed: measuring the other arm means checking that commit out and
    /// running the harness again **in the same session on the same instance**, which is a
    /// script's job and not a flag's. What the flag buys is that the document says which
    /// comparison it belongs to, so a reader cannot pair it with a table from another machine.
    ...
    fn parse() -> Args {
        ...
        while i < argv.len() {
            match argv[i].as_str() {
                ...
                "--baseline" => {
                    a.baseline = Some(argv[i + 1].clone());
                    i += 1;
                }
                "--mixed-seconds" => {
...
fn document(
    ...
    s.push_str(&render::Provenance::gather(args.baseline.clone()).render(ceiling));
    s.push_str(
