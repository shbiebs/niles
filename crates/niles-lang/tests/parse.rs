...
fn the_worked_example_parses() {
    ...
    assert!(
        !d.has_errors(),
        "the worked program must parse:\n{}",
        d.render(src, "demo_bank.niles")
    );
    assert!(prog.items.len() >= 5, "{}", prog.items.len());
    let Item::Schema(s) = &prog.items[0] else {
        panic!("first item is the schema")
    };
...
fn a_ledger_is_a_base_with_a_conservation_rule() {
    ...
    let Item::Schema(s) = &p.items[0] else {
        panic!()
    };
    let SchemaItem::Base(r) = &s.items[0] else {
        panic!()
    };
    ...
    assert!(r
        .rules
        .iter()
        .any(|x| matches!(x, RelRule::Conserve { .. })));
...
fn reserved_words_need_the_raw_escape() {
    ...
    assert!(
        rendered.contains("r#select"),
        "must suggest the escape:\n{rendered}"
    );
...
fn recovery_keeps_the_rest_of_the_file() {
    ...
    let schemas: Vec<&str> = prog
        ...
        .collect();
    assert!(
        schemas.contains(&"good_one") && schemas.contains(&"good_two"),
...
fn an_unguarded_fixpoint_has_no_spelling() {
    ...
    assert!(
        d.has_errors(),
        "unguarded recursion must be rejected at parse time"
    );
