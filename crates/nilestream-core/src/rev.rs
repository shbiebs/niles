...
impl Rev {
    ...
    fn merge_suffix(&mut self, base: &dyn Base, key: &Key, anchor: Epoch) -> Option<Merged> {
        // C11-01's plan check and C11-05(c)'s interval both live here: the base must answer
        // the plan this view installed *before* its deltas are read, and what the walk learns
        // is a `Merged`, not a bare delta. The merge of the two cards is the conjunction.
        self.require_base(base);
