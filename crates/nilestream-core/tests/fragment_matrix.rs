...
impl Base for RawBase {
    ...
    fn delta_rows_at(&self, e: Epoch) -> u64 {
        // **The physical rows in the epoch, counted without building them.** C11-05(a)'s
        // contract: this is what the merge budget is checked against, and a default of
        // `deltas_at(e).len()` would be the defect that card removed — it would also be the
        // *wrong number*, because `deltas_at` returns one row per group and the budget is
        // about the source rows the base would touch.
        self.rows.iter().filter(|(at, _)| *at == e).count() as u64
    }
