...
fn transcript() {
    ...
    doc.push_str(
        "**Generated by `cargo test -p nilestream-server --test psql_conformance -- --ignored \
         transcript`.** Client: libpq's `psql`; which build of it ran is a fact about the host, \
         recorded in `wire-protocol-client.md` and deliberately not compared across hosts. \
         Everything below this line is, and reproduces byte for byte on any of them.\n\n\
    ...
    // The one line of the transcript that is a property of the machine rather than of this
    // server. It lived in the document's header until cycle 10, where two hosts produced
    // byte-identical transcripts under `psql` 16 and 18 and `make reproduce` failed anyway,
    // on the banner. Splitting it keeps the transcript under `git diff --exit-code` — which
    // is what makes the cross-version agreement a checked result rather than an anecdote —
    // and states the host fact where nothing compares it.
    let client = format!(
        "# The client that drove the wire transcript\n\n\
         **Generated by `cargo test -p nilestream-server --test psql_conformance -- --ignored \
         transcript`, beside `wire-protocol-session.md`.**\n\n\
         Client: `{version}`\n\n\
         This file is host-scoped: it records which libpq build produced the transcript and is \
         excluded from the reproduction diff, because a second host with a different \
         PostgreSQL package would differ here and nowhere else. That it differs *only* here is \
         the result: the framing this server emits was accepted identically by `psql` 16.13 on \
         Ubuntu and `psql` 18.6 on macOS, and the transcript beside this file is compared byte \
         for byte on every host.\n"
    );
    let cpath = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../results/wire-protocol-client.md"
    );
    std::fs::write(cpath, client).expect("write the client banner");
    println!("wrote {cpath}");
}
