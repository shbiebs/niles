...
GitHub, on the author's Mac through the desktop bridge, or as files the author attaches — and from
output the author pastes back. Write every request for a run as a complete, copy-pasteable block
with the expected runtime, and say what you will do with the result. If a request goes unanswered,
say in your work order which conclusions rest on it and are therefore provisional. Your work order
is delivered as a file the author places at `docs/audit/cycle-10/astra-work-order.md` and
commits; you do not commit it.
...
structural: claims versus code, the type system and what it proves that the engine re-checks, the
thesis text against the runtime, the lock order against the source, GBS's product model against
what Loan IQ's and Calypso's users would need. In cycle 9 your reading found the receipt that
belonged to no connection, the two windows in two units, and the currency code derived from hash
order — three of the cycle's seven tasks came from that class, and all three were confirmed by
measurement. **That class is what this brief is built around.** The author reconciles the two work
...
2. **Do not soften a finding because you expect Fable to catch it.** State your evidence class on
   every finding — *read from source*, *inferred from a document*, or *measured by the author on
   request* — and never round toward the other auditor.
...
**Instruments first, and now instruments *second* as well.** Before you audit the code, ask the
author to run `bash ~/Documents/niles/docs/audit/cycle-10/preflight.sh ~/Documents/niles
~/Documents/GBS` on the Mac and paste it. Then, for each instrument whose output you intend to
rely on, answer two questions: *would it have caught the defect it is about to look for?* and — the question cycle 9
...
### 2.3 What you cannot do, and what you do instead

You cannot build, run a test, or measure. Fable's container can, and the two of you are read
side by side. When a claim needs a number, write the block for the author and say what each
possible result would mean before it comes back — a request whose interpretation is written after
the answer is a request that cannot be wrong. The Mac's `cargo +1.97.1 clippy` is the author's
oracle, not yours; the container's gate is Fable's. Yours is the source.
...
### 6.6 What only you can do this cycle: read

Fable measures. You read what a measurement cannot reach, and cycle 9 shows the class pays:

- **The lock order, from source, for every path.** `rev_engine.rs` documents O < B < P < V < C
  with S and F as leaves. Enumerate every function that takes two of them and write the order it
  takes them in; then say whether §6.1's candidates (a snapshot handle; a frontier-only guard) can
  be introduced without a path that takes B after V. The source guard covers two functions; the
  order is claimed for all of them.
- **`Base` as a trait, against its implementations.** `nilestream_core::rev::Base` has three
  methods. For `reconstruct`, say what each implementor actually needs from its receiver and
  whether it could be called on an immutable snapshot. §6.1 turns on this.
- **Theorem 4.1 clause (5) and `finish_fold`, line by line.** The execution report claims 5a is
  provable without the generation check. Prove it or refute it in the margin.
- **Every "→ divergence" and every "cannot" in cycle 9's guard specifications and Chapter 3**, for
  the hazard-class-overstated shape (§9).
- **The type facts the engine re-derives** (§6.4), from the typechecker's obligations outward: for
  each obligation `typecheck.rs` discharges, find the runtime line that re-checks it, and classify
  it as a type fact (removable) or a data fact (must stay). Fable will cost each with callgrind;
  you say which ones are on the list.
- **`conserve per (…)` against §6.5's partition question** (LC-41): what the grammar admits today,
  what the typechecker proves about it, and whether a cross-partition *prohibition* is a new
  invariant or a new spelling of an old one.
- **Loan IQ and Calypso onto the 29 rows** (§6.5): extend cycle 9's crosswalk (F-79) with the new
  material, one row per capability, **P / G / A**, and the invariant each absent shape would
  stress. Fable will check your classifications against G3's evidence; you make them.
- **The author's GBS tree.** Through the bridge, `git --no-optional-locks -C ~/Documents/GBS log
  --oneline -3`; if it is at `963e4d9`, the §4 block goes at the top of your work order.
...
1. **Have the gate run** on both trees at the heads in §4 — ask the author for `make gate` on the
   Mac and paste — and record every row. Then ask for `cargo test --offline --workspace
   --no-fail-fast -- --test-threads=8` **three times** and record any test that changes verdict.
   Fable runs the same under load in the container; the two records are compared by name.
...
0. **The preflight output, verbatim, with its admissibility table filled in** — ask the author to
   run `bash ~/Documents/niles/docs/audit/cycle-10/preflight.sh ~/Documents/niles ~/Documents/GBS`
   on the Mac and paste the output. Where a row needs the container, say so and leave it to Fable.
