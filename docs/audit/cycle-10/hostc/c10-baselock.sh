...
#
# WHAT THIS MEASURES
#   The mixed workload at 6r/3w, 9r/5w and 12r/6w on one or two builds: per level, the
#   throughput row, the slowest-16 keyed reads broken into base wait / view wait / view hold,
#   the flight counters, and — once C10-01 has landed — the full `nilestream_lockstats` row
#   with the base lock's READ and WRITE holds reported separately.
#
...
#   the document was wrong by 7.8x; this script exists so that cannot happen again. Run it
#   again, two-armed, after each candidate lands.
#
...
#
# USAGE
#   bash c10-baselock.sh --baseline-only                    the current build only (est. 12 min)
#   bash c10-baselock.sh --candidate c10/04-deferred-merge  two arms, interleaved (25-35 min, cap 45)
...
# Refs, not SHAs baked into the script: a script that names its own commit cannot name it,
# because writing the SHA in changes it. Each ref's resolved SHA is printed in the preflight.
BASELINE_SHA="c7/01-durable-rows"
CANDIDATE_REF=""
BASELINE_LABEL="baseline"
CANDIDATE_LABEL="candidate"

REPO="$HOME/Documents/niles"
OUT="$HOME/c10-baselock-out"
WARMUPS=2
MEASURED=5
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --baseline)  BASELINE_SHA="$2"; BASELINE_LABEL="baseline $2"; shift ;;
...
if [ "$BASELINE_ONLY" -eq 0 ] && [ -z "$CANDIDATE_REF" ]; then
  note "REFUSED: name a candidate with --candidate <ref>, or pass --baseline-only."
...
note "repo            : $REPO"
note "out             : $OUT"
note "baseline        : $BASELINE_SHA -> $(git -C "$REPO" rev-parse --short "$BASELINE_SHA" 2>/dev/null || echo 'UNRESOLVED')  $BASELINE_LABEL"
...
# checkpoint work is C10-04 and, when it is not the arm under test, must not be entangled with
# this measurement. Probing for it
...
replicate() {
  ...
  # table exists.
  sed -n '/E19 mixed/,$p' "$OUT/bench-$arm-$rep.log" | grep -v "connected as\|disconnected after" | sed 's/^/    /'
  return 0
}
...
note "    flights_refused and deferred_merges at 12r6w;"
