#!/usr/bin/env bash
# c10-gates.sh — the correctness gates, on Host C, in the pairing the audit requires.
#
# WHAT THIS IS FOR
#   `c10-baselock.sh` measures. This one decides whether anything may be measured at all: the
...
#
#   Three cycle-10 findings are about gates that were green while checking nothing:
...
#
# USAGE
#   bash c10-gates.sh                      both trees, at their defaults
#   bash c10-gates.sh --niles ~/n --gbs ~/g
#
...
NILES="${C10_NILES:-$HOME/Documents/niles}"
GBS="${C10_GBS:-$HOME/Documents/GBS}"
...
note "niles       : $NILES"
note "gbs         : $GBS"
[ -d "$NILES/.git" ] || { note "FATAL: $NILES is not a git checkout."; exit 3; }
...
# Untracked files are not a refusal here either, for the reason c10-baselock.sh gives: the
...
if ( cd "$NILES" && cargo test --offline --workspace ) >/tmp/c10-gates-niles.log 2>&1; then
  grep -E "^test result" /tmp/c10-gates-niles.log \
    | awk -F'[ ;]+' '{p+=$4; f+=$6; i+=$8} END {printf "  pass %d  fail %d  ignored %d\n", p, f, i}'
  note "  green"
else
  bad "the niles workspace suite"
  grep -E "^test .* FAILED|^error|panicked at" /tmp/c10-gates-niles.log | head -20 | sed 's/^/    /'
  note "  full log: /tmp/c10-gates-niles.log"
fi
...
if ( cd "$NILES" && GBS_ROOT="$GBS" cargo test --offline -p nilestream-core --test downstream_adapter -- --nocapture ) \
     >/tmp/c10-gates-adapter.log 2>&1; then
  if grep -q "PAIRED ADAPTER GATE: NOT RUN" /tmp/c10-gates-adapter.log; then
    notrun "the paired adapter gate opted out (NILES_NO_GBS). A pass produced by an opt-out is not a pass."
    grep "PAIRED ADAPTER GATE" /tmp/c10-gates-adapter.log | sed 's/^/    /'
  else
    note "  green: the adapter compiles against this tree's public traits"
  fi
else
  bad "the paired adapter gate"
  grep -E "PAIRED ADAPTER GATE|error\[E|panicked at" /tmp/c10-gates-adapter.log | head -15 | sed 's/^/    /'
  note "  full log: /tmp/c10-gates-adapter.log"
fi

hdr "3. gbs: the workspace suite, and the adapter crate that is not a default member"
if ( cd "$GBS" && cargo test --offline --workspace ) >/tmp/c10-gates-gbs.log 2>&1; then
  grep -E "^test result" /tmp/c10-gates-gbs.log \
    | awk -F'[ ;]+' '{p+=$4; f+=$6; i+=$8} END {printf "  workspace: pass %d  fail %d  ignored %d\n", p, f, i}'
else
  bad "the gbs workspace suite"
  grep -E "^test .* FAILED|^error|panicked at" /tmp/c10-gates-gbs.log | head -20 | sed 's/^/    /'
  note "  full log: /tmp/c10-gates-gbs.log"
fi
...
if ( cd "$GBS" && cargo test --offline --manifest-path crates/gbs-nilestream/Cargo.toml ) \
     >/tmp/c10-gates-gbs-adapter.log 2>&1; then
  grep -E "^test result" /tmp/c10-gates-gbs-adapter.log \
    | awk -F'[ ;]+' '{p+=$4; f+=$6; i+=$8} END {printf "  adapter  : pass %d  fail %d  ignored %d\n", p, f, i}'
else
  bad "the gbs adapter crate (gbs-nilestream)"
  grep -E "^test .* FAILED|panicked at" /tmp/c10-gates-gbs-adapter.log | head -20 | sed 's/^/    /'
  note "  full log: /tmp/c10-gates-gbs-adapter.log"
fi
...
if ( cd "$NILES" && make generated ) >/tmp/c10-gates-generated.log 2>&1; then
  note "  green"
  grep -E "^  ok: " /tmp/c10-gates-generated.log | sed 's/^/  /'
else
  bad "the generated-document gate"
  head -40 /tmp/c10-gates-generated.log | sed 's/^/    /'
  note "  full log: /tmp/c10-gates-generated.log"
fi
...
if ( cd "$NILES" && make reproduce ) >/tmp/c10-gates-reproduce.log 2>&1; then
  note "  green"
  grep -E "OVER BUDGET|within budget|allocations" /tmp/c10-gates-reproduce.log | head -20 | sed 's/^/    /'
else
  bad "make reproduce"
  grep -E "OVER BUDGET|differs|error" /tmp/c10-gates-reproduce.log | head -20 | sed 's/^/    /'
  note "  full log: /tmp/c10-gates-reproduce.log"
fi
