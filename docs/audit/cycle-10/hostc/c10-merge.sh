#!/usr/bin/env bash
#
# T04.2 — the deferred merge against the pinned control, on Host C.
#
# WHAT THIS RUNS
#
...
#
#   Every refusal this measurement needs already exists in `c10-baselock.sh` and is already
...
exec bash "$here/c10-baselock.sh" --merge-arms "$@"
