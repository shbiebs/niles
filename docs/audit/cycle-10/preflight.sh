...
#
#   bash docs/audit/cycle-10/preflight.sh            # from a clone of `niles`
...
say "cycle-10 audit preflight — $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
...
hdr "C. toolchain"
# **Never let rustup try to download.** `rustc --version` under an unresolvable pin makes rustup
# attempt to install it, and in a container with no egress that attempt hangs for minutes — the
# cycle-9 preflight hung exactly that way the first time it was run here. So the pin is checked
# against `rustup toolchain list`, which reads the disk, and every probe below runs with the
# toolchain named explicitly.
PIN="$(sed -n 's/^ *channel *= *"\(.*\)"/\1/p' "$NILES/rust-toolchain.toml" 2>/dev/null | head -1)"
if rustup toolchain list 2>/dev/null | grep -q "^${PIN:-__none__}"; then
  TC="$PIN"; PIN_STATE="resolves here"
else
...
say "1.97.1           : $(rustup toolchain list 2>/dev/null | grep -q '^1.97.1' && echo 'installed — the author'"'"'s newer-lint gate' || echo 'not installed here — the author runs that gate on the Mac')"
