# Cycle 11 audit brief — **Claude Fable 5.1**
...
execute in a later session. You write no production code into either repository. You read, you run the suites, you run the gates, you write throwaway probes outside the trees, and you keep testing in the cloud container: it is the one place in this project where a deterministic counter can be produced without asking anyone. Commits are yours only for the work order itself, on `c11/00-audit`, bundled to the author as §12 describes.
...
companion brief.** Astra has no shell: it reads GitHub and the author's Mac through the desktop bridge, and what the author pastes, and it asks the author to run things. Its brief leans structural: claims against code, the type system, the thesis text, the decision record, GBS's product model against Loan IQ and Calypso. Yours leans empirical: you measure what Astra can only read. The author reconciles the two work orders afterwards, as in cycles 9 and 10 (`docs/audit/cycle-10/work-order-10.md` §2 is what that looks like: every finding carries the other auditor's verdict — *confirmed*, *confirmed-narrowed*, *not re-read* — and the disagreements are argued out by name). Three consequences:
...
**Try before you ask.** From the container: `GIT_TERMINAL_PROMPT=0 git ls-remote https://github.com/shbiebs/niles` says in one line whether you can read it. If you cannot, ask the author for the narrowest thing that works: **read access only**, to those two repositories and nothing else — a fine-grained personal access token with *Only select repositories* → `niles`, `gbs`, and *Contents: Read-only*; the author supplies it out of band, you use it once to clone, you never write it into an output, and you never push. If access is refused, say so in your work order and audit what you can reach: the bundles under `~/Documents/niles-sync/` on the Mac are complete histories and the author can attach them. **Never let a `git` command in the container reach for the network without `GIT_TERMINAL_PROMPT=0`**: a prompt for credentials hangs a session that cannot answer it.
...
### 3.3 The container (yours alone)

A 2-core shared Xeon with real `fdatasync`. **The tree's pin does not resolve here** —
`RUSTUP_TOOLCHAIN=stable` is 1.95.0 and is how you build; `RUSTUP_AUTO_INSTALL=0`, never let
rustup attempt a download; `CARGO_NET_OFFLINE=true`, `GIT_TERMINAL_PROMPT=0`. **PostgreSQL 16 is
installed but not started**: `sudo -n service postgresql start` first, every session, or the gate
is red on `numeric_binary_oracle` for an environment reason. The Host C scripts run here with
`--repo` and `--out` pointed at your checkout; they report `RUSTUP_TOOLCHAIN=stable, because the
tree's pin does not resolve here`, which is the truth. Absolute figures from here are meaningless;
ratios, counts and verdicts travel.
...
### 6.5 What only you can do this cycle: measure

- **The merge with the walk outside V** (§6.1) — the ratio, in a throwaway worktree, both arms.
- **`the_merge_against_the_arrival_gap`** at gaps 0…8 with a budget below the key count, so
  eviction is in play — the single-threaded control has none.
- **The checked-twice sweep** (§6.3) with per-function callgrind on all four shapes.
- **Every self-test in `docs/audit/cycle-10/hostc/`**, and then the question §10 asks of each
  refusal: what fault would it *not* catch?
- **The load-dependent sweep**: `cargo test --offline --workspace --no-fail-fast -- --test-threads=8`
  three times under two `yes >/dev/null &` hogs, then once pinned to one core at `--test-threads=1`.
- **`gbs` at `01e1d85` against `niles` at `653a369`**: build it, run its gate, run the paired
  adapter gate, and say whether `crates/nilestream-core/tests/downstream_adapter.rs` compiles the
  adapter or a copy of its shape.
- **The revert ledger's reverts** (§1.2): for each candidate, a disposable worktree with the
  revert applied and the suite run, so the ledger's "keep / revert" rows are measured rather
  than argued.
...
1. **The gate on both trees at the heads in §2.3** — run it; record every row; PostgreSQL's absence is an environment fact.
...
A single work order, `docs/audit/cycle-11/fable-work-order.md`, containing:
