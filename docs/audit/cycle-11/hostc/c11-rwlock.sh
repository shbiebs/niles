...
# The probe is shared with every other cycle-11 script (F-11-01: three copies, one repaired).
. "$(cd "$(dirname "$0")" && pwd)/toolchain.sh"
REPO="${REPO:-$(cd "$(dirname "$0")/../../../.." && pwd)}"
pick_toolchain
echo "toolchain             : $TOOLCHAIN_USED"
echo "selected by           : $TOOLCHAIN_HOW"
echo "pin declared          : $TOOLCHAIN_PIN"
