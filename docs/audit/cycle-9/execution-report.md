...
**Seven of seven above-cut tasks landed. Twenty-five of twenty-seven target lines are `done`;
two are not — one failed its pass line, and that failure is the cycle's most useful result; one
asked for an artefact the executor did not produce (C9-02.4).** C9-06.2 predicted ≥ 1.25× read throughput
...
Each target is the work order's §8 wording, verbatim. `done` means the line is true of `dc19a4a`; the load runs cited under C9-00 were taken at `dc19a4a` for this report.

| target (verbatim from the work order §8) | status |
|---|---|
| **C9-00.1** The forced temporary-name collision guard passes without overwriting or deleting another live fixture, and three complete `--no-fail-fast -- --test-threads=8` workspace runs on the executing host show no changed verdict. | **done** — `Temp::candidate(name, salt, pid, seq)` shared by constructor and guard; `create_new(true)` with 64-candidate retry. Three `--no-fail-fast -- --test-threads=8` runs at `dc19a4a` under two `yes` hogs: 1,037 / 1,037 / 1,037 passed, 0 failed, no changed verdict. Re-proved by reversion, §2 R1/R2. |
| **C9-00.2** Every test that spawns a thread and asserts on a count states its precondition as its own assertion message, distinct from the property, and the same four runs (three at 8 threads, one pinned to one core at 1 thread) pass under two `yes >/dev/null &` hogs. | **done** — `frontiers.rs` reader loop bounded on writer progress with a `PRECONDITION UNMET:` message distinct from the property; the sealer test asserts its own gate. The four runs (three at 8 threads under two `yes >/dev/null &` hogs, one `taskset -c 0` at `--test-threads=1`) all 1,037 passed, 0 failed. |
| **C9-01.1** Every mixed row names the readers and writers actually spawned, its start and end epoch, and phase-local (since-reset) lock counters; no printed residual or histogram bound claims time outside its measured boundaries. | **done** — `mixed` asserts spawned-vs-labelled readers/writers; `base_epochs` per row; `select nilestream_lockstats reset` before each level; the "0–2 µs" residual withdrawn from Chapter 3. |
| **C9-01.2** Two overlapping reconstructions each report their own visited-row count and the global total is their sum, asserted by a test. | **done** — `reconstruct_balance_counted`; `two_concurrent_folds_each_report_their_own_visited_rows`. |
| **C9-01.3** Every E19 write is a two-leg transfer with a non-zero amount on both arms, asserted on the statement generator. | **done** — `transfer_statement(id, from, to, i)`; `a_concurrent_write_is_a_two_leg_transfer_with_a_non_zero_amount`. |
| **C9-01.4** Every discrepancy this work order names below the cut line has a `MISMATCH-<id>` at its sentence, and the §9.14.1 miss-rate sentence reads the CSV's column. | **done** — markers placed (`MISMATCH-A9-F12`, `-A9-F15`, `-idem-fingerprint`, `-daemon-checkpoints`); §9.14.1 reads `miss_rate` from the CSV (`thesis/09-evaluation.md:737`). |
| **C9-01.5** `preflight.sh` prints "cycle-9", "five untracked files", real core and memory figures on Darwin, and finds an installed toolchain without `timeout`. | **done** — `preflight.sh` prints "cycle-9", "five untracked files", `hw.logicalcpu` / `hw.memsize` on Darwin, and runs its probes under a Python `subprocess` deadline when `timeout` is absent. |
| **C9-02.1** Each successful reply is written only after every receipt of the statements it answers has returned, and a receipt can be observed by no other connection — proved by the delayed-barrier two-connection test on the daemon's real bytes. | **done** — `Appended { epoch, receipt }`; `Session.receipts` + `take_receipts()`; `the_reply_is_written_only_after_every_outstanding_barrier_is_awaited`, `a_receipt_is_reachable_only_from_the_session_that_appended` on the daemon's bytes. |
| **C9-02.2** A failed barrier is reported as 58030 to the statement that caused it and to no other connection, tested with a failing injected barrier and a concurrent reader. | **done** — `a_barrier_that_never_returns_replaces_the_reply_with_58030`; `not_durable(why)` replaces only the causing statement's reply. |
| **C9-02.3** Admission, sealer and replay agree on one declared window in logical epochs at W−1, W and W+1 under batch sizes 1 and > 1, and the segment record's sequence number is named `batch_seq` everywhere it appears. | **done** — `while order.len() > w { pop }`; `the_window_holds_the_last_w_transactions_whatever_the_batch_size` at W−1/W/W+1 under batch 1 and > 1; `Record::batch_seq` / `Segment::next_batch_seq`; paired GBS `447544d`. Re-proved, §2 R4. |
| **C9-02.4** No path applies a transaction the sink then refuses and continues accepting; the witness for Astra §2.3's window gap either reproduces the applied-undurable state and shows the daemon stops, or is recorded as "not reproduced" with the invariant that prevents it named. | **not done: no separate witness was produced.** C9-02.3 removes the gap by construction — one window, one unit — and the executor reported that; the line asked for either a reproduction of the applied-undurable state showing the daemon stops, *or* a "not reproduced" record naming the invariant, as its own artefact. Neither exists. The invariant is: admission and sealer prune the same per-transaction queue, so no identity is new to one and a duplicate to the other. |
| **C9-03.1** Every single-bit flip in a non-final record's header, body or CRC is refused with the input file's bytes unchanged, and the committed corpus reports zero prefix-with-success cases outside the final record. | **done** — `torn_corpus.rs`: 8,840 flips + every truncation, 7,984 refused / 856 prefix-with-success, all inside the final record; input bytes asserted unchanged. Re-proved, §2 R5. |
| **C9-03.2** `Sequencer::open_bounded` returns `Err(InvalidData)` on an envelope it cannot decode exactly (short, trailing bytes, invalid UTF-8), and no public constructor can start a sequencer with a window built by `unwrap_or_default`. | **done** — `decode_envelope` strict UTF-8, refuses trailing bytes; `open_bounded` uses `recover_seen_checked(..).map_err(..)`; unchecked `recover_seen` / `recover_txns` deleted. |
| **C9-03.3** `docs/SPEC-ENGINE.md` states the self-checking record header, the three-way classification (clean end / torn tail / damage) and the version byte, and the thesis sentence on the record layout carries no `MISMATCH`. | **done** — record-layout and three-way classification tables in `SPEC-ENGINE.md`; Chapter 7's sentence corrected. |
| **C9-03.4** A segment cut exactly at a record boundary is recorded as `LC-36` / `BLOCKED-recovery-tip`: indistinguishable from a shorter log without a retained tip, which is the author's contract to give — never claimed solved by the parser. | **done** — LC-36 / `BLOCKED-recovery-tip` raised, three occurrences in the tree, none claiming a repair. |
| **C9-04.1** Every currency code in the IR and on the wire is the currency's declaration index, tested by lowering a three-currency schema under permuted `HashMap` insertion and asserting identical `LitMoney` codes and wire codes each time. | **done** — `CurrencyInfo::code = cat.currencies.len()` at collection; `currency_codes.rs`, 200 permuted resolutions. Re-proved, §2 R6 (`left: 21, right: 0`). |
| **C9-04.2** `nilestreamd --budget 2,500` (and each other flag with a non-integer) exits non-zero naming the flag and the text. | **done** — `fn flag<T: FromStr>`; `daemon_flags.rs`, 7 process-level refusals. |
| **C9-04.3** Two relations declaring different windows are refused at startup naming both; one relation gives its window deterministically across ten starts. | **done** — `declared_idem_window -> Result<Option<u64>, String>`, names sorted; ten-start determinism tested. |
| **C9-04.4** `txn idem("k", window: 30.days)` is NL0218 in every corpus that used it (25 files, none under `docs/audit/`); a `txn idem(..)` posting to a relation without an `idem` column is NL0219; the mutant is refused; `ignored_windows` no longer exists. | **done, with one residue** — NL0218 across the corpus (82 sites; the only `.niles` files still carrying `window:` are the two mutants that must); NL0219 with `idem_window_without_a_key.niles`; `ignored_windows` gone from the code. Residue: `crates/niles-interp/src/lib.rs:401` still names it in a doc comment. |
| **C9-04.5** `nilestreamd` with neither `--durable` nor `--volatile` exits non-zero; with `--volatile` the banner says VOLATILE; a schema without an `idem` column on the wire relation is refused unless `--idem-window` is given. | **done** — LC-16 and LC-35 refusals; `--volatile` banner; `daemon_flags.rs`. |
| **C9-05.1** `schema_parses` after 2,000 inserts in one session at one schema epoch reads 1; after a schema change it reads 2; tested. | **done** — `Session.currencies` keyed by schema epoch; `schema_parses` a `nilestream_stats` column. Re-proved, §2 R7 (`left: 2000, right: 1`). |
| **C9-05.2** `Session::insert` on the `checked-twice oltp` probe costs at most 45,000 instructions per insert under callgrind on the executing host (was 113,500), stated with the command. | **done** — `checked-twice.rs`, `make checked-twice`, before/after in `docs/BENCHMARK.md`; 113,500 → ≤ 45,000 instructions per insert on the container. |
| **C9-05.3** The four SQLSTATE refusals on the insert path each still have a passing test. | **done** — unchanged, green. |
| **C9-06.1** Every concurrent read equals an independent fold at its own requested anchor while a delayed reconstruction overlaps an `advance`, and same-key same-anchor readers share exactly one fold without holding V while folding or waiting — proved by the latch-driven two-phase differential whose precondition counter (`pinned_installs + deferred_merges + pending_joins > 0`) is asserted. | **done** — `rev::two_phase::a_flight_that_overlaps_an_advance_is_joined_shared_and_installed_pinned`; precondition asserted. Reversions §2 R8–R11. |
| **C9-06.2** At the corrected 12r6w Host C point, Pending delivers at least 1.25× the baseline median read throughput under the ≥ 10% ∧ ≥ 3 pooled-MAD gate, with no contract or durability regression and the slowest-16 table's `view_wait_us` maximum reported beside it (expected: ≥ 2.5× and < 2 ms; reported, not required). | **not done: measured and failed. 1.018× (+1.8%)** at 12r/6w on Host C; clears 3.28 pooled MADs, fails the ≥ 10% arm. No contract or durability regression. `view_wait_us` maxima reported in §11. The baseline the pass line was written from is refuted — §11. |
| **C9-06.3** `nilestream_stats` reports `pending_joins`, `uninstalled_folds`, `pinned_installs`, `flights_refused`; the 12r6w run shows `pending_joins > 0`; `MISMATCH-pending-unreachable` is removed only then. | **done** — four columns; median 14,377 `pending_joins` at 12r/6w on Host C; the marker is struck from Chapter 3 (count 0). |
| **C9-06.4** Theorem 4.1's proof carries the Pending clause; §3's lattice paragraph and its read-path rules (3:160, 3:162) state the exact-anchor interval `stamp ≤ a ≤ effective`, the join on equal anchors and the generation-owned install as implemented. | **done** — Theorem 4.1 clause (5), 5a–5c; Chapter 3's lattice, read-path and upquery paragraphs rewritten. |
...
| the same, three times at `--test-threads=8` under two `yes >/dev/null &` hogs, then once `taskset -c 0` at `--test-threads=1` | 1,037 / 1,037 / 1,037 / 1,037 passed, 0 failed, **no changed verdict** (at `dc19a4a`) |
