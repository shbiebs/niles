# Cycle 9 audit brief — **Claude Fable 5.1**
...
without asking anyone. Commits are yours only for the work order itself, on a `c9/00-audit` branch,
bundled to the author as §10 describes.
...
companion brief.** Astra has no shell: it reads GitHub and what the author pastes, and it asks the
author to run things. Its brief leans on claims-versus-code, the type system, the thesis text, and
GBS's product model against Loan IQ and Calypso. Yours leans empirical: you *measure* what Astra can
only read, and you re-derive every number the executor reported in cycle 8 before you build on it.
...
2. **Do not soften a finding because you expect Astra to catch it.** Independent corroboration is
   the only reason cycle 6's three sharpest findings were trusted.
3. **§8 lists things you must both check independently.** Do those even knowing they are duplicated.

**The prior cycle's executor was also a Claude session, and the auditor who wrote its work order
was you.** Cycle 8's execution report records that the work order's own hypothesis about the read
tail was wrong (§11 there; §5.6 here). Treat everything in `docs/audit/cycle-8/` as claims to
verify from the source and by measurement, not as facts to build on — including this brief.

**Instruments first.** Before you audit the code, run `docs/audit/cycle-9/preflight.sh` and read
`run6.sh`; then decide whether each instrument you intend to use would have caught the defect it is
about to look for. Cycle 8's `run6.sh` failed twice on the reference host before it produced a
number, both times for reasons that were in the script and not in the engine.
...
- **The E18 apportionment, re-derived** (§8.3), plus two rows nobody has: bytes per cached
  `Lowered` in the plan cache, and the slot map's bytes per hole at 100k keys read under a 2,500
  budget — the residual T-05 left and §6.2 is about.
- **The concurrent differential with a long reconstruction.** Make `SharedBase::reconstruct` slow
  enough (a busy fold over a synthetic history, not a sleep) that it overlaps `advance`, and
  confirm the differential still passes at `5eb0bb2`. That is the baseline the §6.1 repair's guard
  must fail against when the reconstruction is moved outside the lock and the join is wrong.
- **A torn-segment corpus prototype** (§6.4) as a throwaway probe: how many offsets of a
  ten-record segment, truncated or bit-flipped, does `with_durable_bounded` refuse, and how many
  does it recover? Any "recovered a prefix" is a finding today, not a task for later.
- **The checked-twice sweep** (§6.3) with `callgrind` *per function* rather than per process:
  instruction counts attributed to `declared_currencies`, `parse_insert`, `minor_units`, the
  verifier and the fold, on E16's `oltp` and `point` statements. Per-function attribution is stable
  where the whole-process total is not.
- **The load-dependent sweep** (§8.1): three runs at `--test-threads=8`, and then once at
  `--test-threads=1` with `taskset -c 0` if the container allows it — a test that passes only when
  it has the machine to itself is the class.
...
1. **Run the gate yourself** on both trees at the heads in §4 and record every row
   (`RUSTUP_TOOLCHAIN=stable`; PostgreSQL 16 up for `numeric_binary_oracle` — start it, do not
   report its absence as a failing test). Then run `cargo test --offline --workspace --
   --test-threads=8` **three times** and record any test that changes verdict between runs: the
   load-dependent class from §5.3 has been swept exactly twice.
...
0. **The preflight output, verbatim, with its admissibility table filled in** — `bash
   docs/audit/cycle-9/preflight.sh <niles> <gbs>` where you can run it; where you cannot, ask the
   author to run it on the Mac and paste the output, and say so.
...
6. **Every Host C script**, named, with its expected runtime, its retarget-and-refuse clause, its
   ...
   now**" marker where its result is needed.
