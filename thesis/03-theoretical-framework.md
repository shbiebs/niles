...
⊥: the runtime asserts nothing about k. Hole(e′): the key was evicted; the runtime retains the *version* it held, not the value. Pending(e′): an upquery anchored at e′ is in flight; readers wait or degrade per contract. **`MISMATCH-pending-unreachable`**: the runtime never enters this state — `Slot::Pending` is constructed nowhere in `nilestream-core` — so the engine implements three of these four. The consequence is measured rather than theoretical: because there is no in-flight state to join, a reconstruction runs while its reader holds the view's lock, and on the reference host a mixed workload's read throughput *falls* from 94,000/s at six connections to 20,000/s at twelve, with the slowest reads waiting 22 ms on that lock (`docs/audit/cycle-8/lc-23-attribution.md`). **The companion claim that those reads spent "0–2 µs on everything else" is withdrawn.** It came from a column computed as `total − (base_wait + view_wait + view_hold)` over four timestamps taken consecutively inside one function, so the three parts partition the fourth by construction and the remainder is the truncation of four microsecond divisions — bounded by 2 µs whatever the wire, the framing or the scheduler did, because none of them lies between those timestamps (cycle 9, A9-F07). The view wait itself is a measured difference and stands; the exclusion of everything else was arithmetic wearing the clothes of a measurement. `select nilestream_lockstats` now carries two further scopes — the statement and the connection — whose boundaries do contain those terms. The state exists in the lattice for exactly this reason and the engine has yet to reach it. Present(v, e′): v = V\*(e′)[k], certified.
