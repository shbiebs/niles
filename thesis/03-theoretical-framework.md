...
⊥: the runtime asserts nothing about k. Hole(e′): the key was evicted; the runtime retains the *version* it held, not the value. Pending(e′): an upquery anchored at e′ is in flight; a reader asking for that same anchor joins it, and a reader asking for any other anchor reconstructs independently. **The engine reached three of these four states until cycle 9.** `Slot::Pending` was constructed nowhere in `nilestream-core`, and not by oversight: reconstruction happened *inside* the read, while its caller held the view's lock, so there was no moment at which a second reader could arrive and find a flight to join, because no second reader could run at all. The consequence was measured rather than theoretical — a mixed workload's read throughput *fell* from 94,000/s at six connections to 20,000/s at twelve on the reference host, with the slowest reads waiting 22 ms on that lock (`docs/audit/cycle-8/lc-23-attribution.md`). C9-06 splits the read in two: `Rev::begin_read` decides under the view lock and returns, the caller folds with **no lock held**, and `Rev::finish_fold` takes the view again to install. The fourth state is what a second reader now finds in between, and the runtime counts what it does about it — `pending_joins`, `uninstalled_folds`, `pinned_installs` and `flights_refused` are columns of `select nilestream_stats`. Under the in-process differential (four readers, one writer, one advancer, 1,600 reads) the run records 82 joins and 64 pinned installs; over the wire, at four readers and two writers against a budget of 50 over 400 accounts, one three-second level records 1,228 joins, 100 uninstalled folds and 26,305 pinned installs. **`MISMATCH-pending-unreachable`** is narrowed to what remains unmeasured: the *throughput* this is predicted to recover, at 12r6w on the reference host, which is a Host C measurement (C9-06.2, `c9-pending.sh`) and is not yet taken. The container figures above are a two-core sandbox and are evidence that the mechanism runs, not evidence about its speed.

**One of those numbers is a finding rather than a report.** 26,305 pinned installs against 1,228 joins says that under a moving frontier almost every reconstruction lands below `applied` and therefore enters pinned — serving one anchor and then reconstructing again. The pinned install is *sound*, which is why it ships first; it is also the weakest of the two available landings, and the deferred merge (§3's upquery rule, `deferred_merges`) is what would let those entries land current instead. On this evidence that is not a micro-optimisation but the difference between a reconstruction that serves one read and one that serves a key until the next delta touches it. **The companion claim that those reads spent "0–2 µs on everything else" is withdrawn.** It came from a column computed as `total − (base_wait + view_wait + view_hold)` over four timestamps taken consecutively inside one function, so the three parts partition the fourth by construction and the remainder is the truncation of four microsecond divisions — bounded by 2 µs whatever the wire, the framing or the scheduler did, because none of them lies between those timestamps (cycle 9, A9-F07). The view wait itself is a measured difference and stands; the exclusion of everything else was arithmetic wearing the clothes of a measurement. `select nilestream_lockstats` now carries two further scopes — the statement and the connection — whose boundaries do contain those terms. The state exists in the lattice for exactly this reason and the engine has yet to reach it. Present(v, e′): v = V\*(e′)[k], certified.
