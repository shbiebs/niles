...
⊥: the runtime asserts nothing about k. Hole(e′): the key was evicted; the runtime retains the *version* it held, not the value. Pending(e′): an upquery anchored at e′ is in flight; a reader asking for that same anchor joins it, and a reader asking for any other anchor reconstructs independently. **The engine reached three of these four states until cycle 9.** `Slot::Pending` was constructed nowhere in `nilestream-core`, and not by oversight: reconstruction happened *inside* the read, while its caller held the view's lock, so there was no moment at which a second reader could arrive and find a flight to join, because no second reader could run at all. C9-06 splits the read in two: `Rev::begin_read` decides under the view lock and returns, the caller folds with **no lock held**, and `Rev::finish_fold` takes the view again to install. The fourth state is what a second reader now finds in between, and the runtime counts what it does about it — `pending_joins`, `uninstalled_folds`, `pinned_installs` and `flights_refused` are columns of `select nilestream_stats`. On the reference host, at twelve readers against six writers over thirty seconds, the served daemon records a median 14,377 joins, 495 uninstalled folds and 311,921 pinned installs. The state is reached, this section no longer describes a mechanism the runtime lacks, and the marker that stood against it since cycle 8 is struck rather than narrowed (cycle 9, C9-06.3).

**What it is not is a speed result, and the measurement that was supposed to establish one refuted its own premise instead.** The claim this repair was undertaken against — a mixed workload's read throughput falling from 94,038/s at six connections to 20,507/s at twelve, with the slowest reads waiting 22,160 µs on the view lock — was *inferred from a document*, and the corrected harness does not reproduce it: the same predecessor build measures 147,306 / 156,686 / 160,838 reads/s across those three levels, **rising** with concurrency, 7.8× the collapsed figure at twelve, with a slowest-read view wait of 788 µs rather than 22 ms. There was no collapse to recover, and the two-phase read is worth **+1.8%** at 12r/6w (1.018×, three pooled MADs, against a 1.25× pass line it does not approach). What differed was the harness, not the engine: one daemon for a whole session with lock histograms never reset, lifetime rather than level-local counters, and a keyed read that folded the base twice on 87.4% of attempts before the certification interval was repaired. **The tail this engine has at twelve connections is a base-lock tail** — in the slowest-read tables of *both* arms, `base wait` is the dominant term and often the whole of it — so the attribution to the view lock is withdrawn and LC-23 is reopened against the base. The two-phase read is kept on the grounds above: the lattice is complete and the hold is bounded. No cell of the phase diagram is drawn from it. (`docs/audit/cycle-9/hostc/c9-pending-results.md`.)

**One counter is a finding in its own right.** Against roughly 4.9 million reads in that level, a joined read is 0.29% and a pinned install is 6.35%: under a moving frontier, twenty-two reconstructions land pinned for every one that is shared. A pinned entry serves the single anchor it was asked for and is rebuilt by the next reader that needs the key fresh. The pin is *sound*, which is why it ships; it is also the weaker of the two available landings, and the deferred merge (§3's upquery rule, `deferred_merges`, zero here) is what would let those entries land current instead. On these ratios that is not a micro-optimisation, and it is worth considerably more than the join is. **The companion claim that those reads spent "0–2 µs on everything else" is withdrawn.** It came from a column computed as `total − (base_wait + view_wait + view_hold)` over four timestamps taken consecutively inside one function, so the three parts partition the fourth by construction and the remainder is the truncation of four microsecond divisions — bounded by 2 µs whatever the wire, the framing or the scheduler did, because none of them lies between those timestamps (cycle 9, A9-F07). The view wait itself is a measured difference and stands; the exclusion of everything else was arithmetic wearing the clothes of a measurement. `select nilestream_lockstats` now carries two further scopes — the statement and the connection — whose boundaries do contain those terms. The state exists in the lattice for exactly this reason and the engine has yet to reach it. Present(v, e′): v = V\*(e′)[k], certified.

**A Present entry is certified over an interval, not at a point.** e′ is the entry's *stamp* — the epoch its value was computed at — and the entry received no delta for k between e′ and the frontier the view has applied through, which is what makes it resident and current. So v = V\*(e)[k] for every e in the closed interval [e′, eff(k)], where eff(k) is the entry's effective version (Section 4.2's inheritance rule: the later of e′ and the view-wide applied epoch, for an entry that is not pinned to a historical anchor). Writing Present(v, e′) with a single epoch is the abbreviation this chapter uses throughout, and it is harmless in the lattice — the ordering and the join are over the stamp — but it is not what the runtime asserts, and reading it as a point is what made the first implementation of `Rev::read` answer at eff(k) rather than at the anchor it was asked for. Theorem 4.1's step (3) states the interval form, and Chapter 9 records what the point reading cost: 87.4% of keyed reads over the wire were served correctly, discarded by the caller for carrying the wrong end of their own interval, and folded again from the base.
...
**Write path.** admit → validate(C, idempotency, authorization) → sequence into open epoch → seal(τ or size) → hash → fsync (and quorum, in replicated profiles) → publish vis. Two things this diagram asserts were true of the design and not of the served daemon until cycle 9, and both are now repaired rather than marked. **The acknowledgement is bound to the append that earned it.** `RevEngine::append` placed its durability receipt in one vector owned by the *engine*, and the connection loop drained the vector rather than its own token: between one connection's append and its drain, another connection's drain took the first one's receipt, so the first was acknowledged with nothing to wait on while its record's barrier was still in flight, and a failed barrier was reported to whichever connection happened to be draining. Visibility was still gated — no read observed the epoch — so the *ordering* above held; what did not hold was the acknowledgement's meaning (A9-F01, closed by C9-02: `append` returns `Appended { epoch, receipt }` and the receipt is held on the session, which belongs to one connection). **And the two idempotency windows count the same unit.** The base's admission index is bounded by the last W *transactions*; the sealer's was bounded by W *segment records*, and a record is a batch of up to 4,096 transactions, so one declaration produced two windows differing by up to that factor and a retry in the gap was new to admission and a duplicate to the sink — an applied transaction that cannot be acknowledged (A9-F02, closed by C9-02: the sealer prunes by its per-transaction queue, and the record's number is named `batch_seq` so the two coordinates cannot be read as one). Reservations (Section 3.19) are ordinary appended rows admitted under the same rule.
...
**Read path.** Given (view, key, contract, session): compute the *required anchor* a from the contract and session watermarks (ℓ₀: any e ≥ vis − K; ℓ₂: ≥ session's last write epoch; ℓ₃: the read set's chosen e; ℓ₅: vis(t)). Let the slot be Present(v, e) and let its *effective* anchor be eff = e if the entry is pinned and max(e, applied) otherwise, where `applied` is the frontier the view as a whole has been maintained through and the inheritance is sound exactly for an entry that was resident across every epoch in (e, applied]. Then: **serve (v, a) iff e ≤ a ≤ eff** — the closed *certification interval*, on which the value is constant, so the answer is exact at every anchor inside it and is returned stamped with the anchor that was asked for, not with eff. An anchor above eff has deltas the entry has not seen, and an anchor *below* e has deltas the entry must not show; both reconstruct. If Hole or ⊥, upquery at a. If Pending(a′): **join the wait iff a′ = a**, and otherwise upquery at a independently, because a flight at another anchor is folding a different prefix and its answer is not this reader's. Equality and not a′ ≥ a: a value certified at a later anchor includes writes this reader's snapshot excludes, and serving it would be the wrong-anchor answer the interval exists to make impossible. This rule was `anchor: eff` before cycle 8 and every caller in the workspace independently wrote the same compensating branch — *if the anchor came back different, discard and refold* — paid on 87.4% of keyed reads over the wire under concurrent writers.

**Upquery.** Evaluate the circuit in pull mode along the provenance-derived paths against the *frozen prefix at a*, **holding no lock on the view**. Because the prefix is immutable, no update can interleave with the reconstruction in a way that changes its result: the answer (value, a) is exact whatever else happened while it was computed, and the reader that requested it is served unconditionally. What is conditional is the *install*. A flight is authorised under the view lock and carries an immutable (k, a, g), where g is a generation the view issues and never reuses; on completion the slot is filled as Present(value, a) **iff the view still holds a flight for k at generation g**. A completion whose generation has been superseded — its owner was cancelled and a later reader took the key over — still answers its own caller and writes nothing, because overwriting a newer resident entry moves that entry's stamp backwards and clears a flight record belonging to somebody else. Concurrent epoch application to the same key does not resolve by "the higher anchor wins": an epoch sealed during a flight is *skipped* for that key rather than folded into the marker, since a Pending slot holds no value to fold into, and if the frontier moved past a while the flight was out the install is **pinned** — the entry keeps its own anchor a and does not inherit `applied`, because it never saw the deltas in between. Folding those deltas into the landing result instead of pinning is a strictly better install and a separate change; the counter `deferred_merges` reports it and is zero here.
...
where U is the reconstruction/coordination term determined by miss rate under π and m and by Z — **`MISMATCH-A9-F12`: two qualifications this equation needs and does not carry.** The served daemon builds its ledger with `checkpoint_interval = 0`, so every reconstruction it performs folds a key's entire history and U is history-shaped for exactly the configuration Chapter 9's wall-clock tables were measured on; and even with checkpointing, the *physical* index is keyed by account while checkpoints are keyed by (account, currency), so a fold visits every row of the account in every currency since the last checkpoint of its own currency. The C/2 + 1 bound is a bound on target-key rows, not on rows visited. Cycle 9's C9-07 makes the interval a flag with a stated default and narrows or repairs the index — and Φ is a rung multiplier: Φ(ℓ₀…ℓ₂) = O(1) (anchor bookkeeping only); Φ(ℓ₃) = O(1) plus snapshot pinning memory; Φ(ℓ₄) = O(contention) *for read-write transactions, and O(1) for read-only ones, where ℓ₄ coincides with ℓ₃*; Φ(ℓ₅) = Θ(freshness), since every read must observe vis(t). **No term depends on the base length n** — history enters only through per-key update counts, which is a workload property. That is the claim H-S3 operationalizes.
