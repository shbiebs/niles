...
**A second instrument, and why this architecture suits it.** Differential testing against 𝒪 checks *values*; it does not check *isolation*. The distinction is not academic: a published analysis of a distributed SQL database found that its conservation-of-money tests passed consistently while a causal-reverse anomaly showed the system was serializable but not strictly serializable. The correctness programme therefore adds black-box anomaly inference in the style of Elle, which infers an Adya-style dependency graph from client-observed histories and detects the cycles that define G0, G1a–c, G-single and G2, including real-time and per-process edges, and which is *sound*: an anomaly it reports is present in every interpretation of the observation.
...
Formally, the happens-before order generated by seal events makes every access to sealed data a read of an immutable location; hence no data race can involve the base, and the residual proof obligations are exactly two lock disciplines — small enough for exhaustive interleaving checks. The implementation encodes this in the type system: sealed data is reachable only through shared immutable references, so the absence of `&mut` to prefix data is a compile-time property rather than a convention. Rust's guarantee here is the affine-ownership one formalized by RustBelt, whose central invariant is that "aliasing and mutation cannot occur simultaneously on any given location" [Jung et al., POPL '18]. Section 5.8 gives the verification plan; Section 9.10 the acceptance criteria.
