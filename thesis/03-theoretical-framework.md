...
**Write path.** admit → validate(C, idempotency, authorization) → sequence into open epoch → seal(τ or size) → hash → fsync (and quorum, in replicated profiles) → publish vis. **`MISMATCH-A9-F01`: the served daemon's acknowledgement is not yet bound to this sequence.** `RevEngine::append` places its durability receipt in one engine-wide vector and the connection loop drains *the vector*, not its own receipt, after `Session::handle` returns; between one connection's append and its drain, another connection's drain takes the token. The client is then told the transaction committed while its record's barrier is still in flight on a different connection, and a barrier that fails is reported to whichever connection happened to be draining. Visibility is still gated — no read observes the epoch — so the ordering of *this* diagram holds; what does not hold is the acknowledgement's meaning. **`MISMATCH-A9-F02`: two structures called windows count in different units.** The base's admission index is bounded by the last W *transactions*; the sealer's is bounded by W *segment records*, and a record is a batch of up to 4,096 transactions. A retry arriving in the gap is new to admission and a duplicate to the sink. Both are repaired in cycle 9's C9-02. Reservations (Section 3.19) are ordinary appended rows admitted under the same rule.
