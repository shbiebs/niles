...
- κ is the *serve contract*: consistency rung, freshness bound (K, T), materialization mode (Section 4.6), memory budget share, retention, lineage mode, and confidentiality. Of these the compiler checks and the runtime reads the rung, the freshness bound, the materialization mode and retention; the lineage mode is specification and nothing implements it (Remark 4.1.2).

The REV supports six operations: **read**(k, session) per κ; **apply**(Δₑ) folding a sealed epoch into resident entries; **evict**(k) mapping Present(v, e) ↦ Hole(e); **upquery**(k, a) mapping Hole/⊥ ↦ Present(Q(S(a))[k], a); **version**(k) returning the anchor without the value; and **explain**(k), which would return the lineage of the current entry at its lineage mode and is specification — no lineage mode exists, so the operation has nothing to vary.
...
**(5) The Pending clause.** The statement above is proved for the three-state lattice, and the runtime now reaches the fourth: between the moment a reader is authorised to reconstruct k at a and the moment its result is installed, M(k) = Pending(a) and the fold runs with no lock on M. Three sub-claims close the gap, and each is an interleaving the proof must survive rather than exclude. *(5a) The served answer does not depend on M at all.* ρ_{k,a} is evaluated against S(a), which clause (2) shows is determined by (Q, a, k); so every reader is served Q(S(a))[k] whether or not its result is ever installed, and a reader that finds a flight at a′ ≠ a and folds independently is served the same value it would have been served with no flight present. A reader that finds a flight at a′ = a and joins it receives that flight's answer, which is Q(S(a))[k] by the same clause — joining is a saving, never a semantics. *(5b) Pending admits no delta.* Pending(a) carries no value, so apply cannot advance it; the epoch is skipped for k exactly as for a non-resident key, and the certification invariant is vacuous at k for the duration. Writing Δ_{e}[k] into the marker would assert M(k) = Present(Δ_e[k], e), which violates the invariant unless k's history is empty. *(5c) An install into a moved frontier is pinned.* If apply advanced the view to e > a while the flight was out, the landing entry Present(v, a) satisfies v = V\*(a)[k] but was not present for the epochs in (a, e], so it is marked not-certified-through-`applied` and its effective anchor is a. It therefore serves only the interval [a, a], which by clause (3) is exact, and any read above a reconstructs. Pinning is what keeps the invariant an invariant; the alternative — folding (a, e]'s deltas for k into the landing value and installing at e — is also sound by (H-F2) and is a strictly better install, reported as `deferred_merges` and not taken here. Finally, an install is conditioned on the flight's generation, which is an identity the view issues once and never reuses: a superseded completion writes nothing, so no interleaving of authorise/cancel/reauthorise can leave M(k) certified at an anchor whose deltas the entry did not see. Absence is unaffected in either direction: cancellation restores the exact slot the marker replaced, version and all, so a Hole never degrades to ⊥. ∎

**Open case 4.1.α (non-linear operators).** For Q ∉ Q_lin the theorem is *not* claimed here. Reconstruction remains pure and total — clauses (1) and (2) of the proof do not use linearity, and the upquery paths exist for every Niles-expressible circuit — but step (3) fails: for an equijoin or a `max`, the delta to a resident key's output depends on input rows belonging to keys that may have been evicted, so a resident entry cannot in general be advanced without either re-reading the base (making apply an upquery, which changes the cost model of C3) or retaining state for absent keys (which is full materialization by another name). The honest statement of the general case is a conjecture with a stated shape: *for every circuit, evict-then-reconstruct is observationally identical to never-evict, provided every advance either is linear per key or is replaced by a reconstruction at the same anchor.* Nothing in Chapter 9 depends on the open case, because the runtime refuses to install a circuit outside Q_lin — **`MISMATCH-A9-F15`: `Runtime::install` examines each *output* node only, checking that it is a keyed `Sum` or `Count`, and never walks the graph feeding it, so a `sum` over a join installs and is then advanced by per-key deltas it is not entitled to. The daemon's own read path is narrower (`FoldPlan::filters_only`), so no wire query reaches the gap; the core API, which the GBS suite and every in-process test use, does. The refusal this sentence relies on has to be made true of the whole reachable graph (cycle 9, C9-08)** — and Corollary 4.1.1 is unaffected: a per-(account, currency) balance is a keyed `sum`, which is in Q_lin by Definition 4.1.1. Section 12 records the general case as the first item of future work.
...
*(c) SQL-fragment translation.* There is a **total, denotation-preserving** compilation of SQL-Core — the fragment stated in Appendix H, covering SELECT-FROM-WHERE with bag semantics, inner/left/right/full joins, grouping and aggregation, set operations, nulls with three-valued logic, ordering and literal-bounded limits — into IR: for every SQL-Core query and every finite instance, the compiled circuit evaluates to the Z-set the query denotes, and where a query has a Niles pipeline spelling the two evaluate alike. *Proof:* by structural induction on the fragment's grammar, with the induction in Appendix H and each production witnessed by a case in the golden corpus. ∎

Three narrowings against an earlier statement, each forced by what the corpus found. **α-equivalence of circuits is not claimed** — the corpus compares answers, and two lowerings of one query may legitimately differ in shape. **DDL, DML and TCL are not in the claim**: a `CREATE TABLE` denotes no Z-set, so no statement of this form is true of it. And **SQL-Core is now defined as what the compiler accepts**, determined by putting each construct through `nilesc` rather than by listing what a translation ought to cover; Appendix H tabulates the refused forms with their codes, and `CROSS JOIN` is among them because the IR has no product operator — until this cycle it lowered to the keyed inner join and silently answered a different query.
...
The seven contributions interlock. C1 makes partiality *safe* — exactness at any anchor for the linear keyed fragment, and conservation across eviction; lineage is a conditional remark over RA⁺ with nothing built to exercise it. C3 *prices* every rung above that floor and shows the price is workload-shaped. C2 *bounds* the whole approach: it proves that the top rung's cost per miss is forced by the rung, and gives the condition under which a break-even against full materialization exists at all. C5 *would decide*, turning the bound into a runtime policy — it is a specification, not a system, and its competitive claim is withdrawn. C4 *transfers* what it can to programs, making conservation, currency safety and authorization typing judgements — for the blocks the solver decides, with the rest handed to the commit rule and said so. C6 *generalizes*, replacing an unprovable slogan about SQL with three theorems and a construction over the fragment the compiler accepts, which is now defined by what the compiler accepts rather than by what a translation ought to cover. And SC7 *bounds the bound*: it supplies the checkpointing mechanism without which C3's cost law is false for skewed workloads, and it is the one contribution here that an experiment produced rather than confirmed.

Niles and Nilestream exist to make each claim falsifiable: the reconstruction theorem is attacked by the conservation suite under fault injection (H-S4), the cost theorems by the phase-diagram and history-scaling experiments (H-S2, H-S3), the optimizer by comparison against fixed policies and an offline optimum (H-S7 — *which has nothing to measure, because the optimizer is not built*), the generality claim by a non-financial conserved-quantity domain that required no compiler change (H-S8, run: §9.11.1), and everything by differential testing against the executable oracle. The theory is the contribution; Chapters 5–9 build and specify the test.

**And the tests found things in the theory, which is the arrangement working.** Writing the SQL corpus's missing cases found that right and full joins evaluated to nothing and cross joins answered the equi-join; running H-S8's falsifier found a three-letter restriction on grades in the lexer; counting E2's two sides found that they were one. Each is reported where it belongs rather than repaired quietly, because a theory whose instrument never contradicts it is a theory nobody has instrumented.
