...
**The falsifier has now been run, and it half-passed.** Section 9.11.1's non-financial conservation domain exists: `examples/inventory.niles` conserves units of a SKU across warehouses, is *proved* to conserve by the currency-row solver, posts and seals through `nilesc run`, and required no change to the compiler, the IR or the runtime — a test asserts that none of them contains the word `inventory`, `widget`, `sprocket`, `warehouse` or `sku`. The machinery this section calls general is general.

What the falsifier found is one line in the lexer: a money literal's grade must be exactly three lowercase letters, so `5 widget` does not lex and every quantity in that file has to arrive as a parameter. A banking assumption survives in the *notation* while the machinery underneath it is domain-neutral — which is precisely the shape of the finding this section already predicts for the seven keywords, arriving in the one place nobody had looked. It is recorded as `MISMATCH-T-22-currency-literal` and left unapplied: widening the lexer to make H-S8 pass would be a change to the language rather than a measurement of it.
