...
| **MySQL wire** (`nilestream-server::mysql_wire`) | **Built** — packet framing with sequence numbers, length-encoded integers, handshake, `OK`/`ERR`, column definitions and text rows; money is `NEWDECIMAL`, never `DOUBLE` |
