...
Conservation extends because each leg balances per currency locally (Definition 3.7), so no interleaving of failures can strand value, and an aborted reservation leaves no postings. The protocol's novelty budget is deliberately small — it is two-phase commit hardened by epoch alignment and consensus-replicated participants — because the thesis's claims live elsewhere. Where a cheaper design is available it is preferred: the deterministic-execution literature shows that pre-agreeing the input order removes the need for a commit *vote* [Thomson et al., SIGMOD '12; Lu et al., PVLDB '20], and the extent to which that technique can subsume the reservation step here is an implementation question this phase settles empirically rather than a claim made in advance.
