...
**Discharged, and tested.** The workspace contains **no `unsafe` block**, and sealed epochs are reached only through shared references. A drift test asserts the first at every build, in both repositories, because "no unsafe" is the kind of claim that is true until one line makes it false. The test scans the whole repository rather than the workspace, and permits exactly one file by name: `tools/memprobe/src/alloc.rs`, the counting allocator the E18 memory experiment installs. `GlobalAlloc` has no safe implementation, so the choice was between an instrument and an unqualified claim; the instrument sits **outside** the workspace, is linked into nothing that ships, and is allowed by name so that a second file wanting the same licence must be added by someone who has argued for it. The claim about the system is unchanged.
...
### 9.14.0 Memory, counted (E18)

`results/E18-memory.md`, generated by `tools/memprobe`; nothing in it is typed by hand and
nothing in it depends on the host.

Until this experiment the memory half of the efficiency goal had **no instrument at all**:
no allocation counter, no bytes-per-posting figure, no threshold in any gate. Every number
in §§9.1–9.4 is counted work — base rows read, deltas applied, resident entries — which says
what the algorithm does and nothing about what it costs to hold. "Efficient in memory" was
therefore a goal with no status: an improvement could not be claimed and a regression could
not be caught, and the reader had no way to tell which.

E18 reports, per scenario, the allocations and requested bytes per operation, what the region
kept, and its high-water mark. Allocation counts are *exact within a build profile*, which is
why they and not wall clock are what `make memory` asserts: a two-core container moves a
wall-clock median by tens of percent between invocations of one probe, and a gate that fails
at random is a gate that gets deleted. What the gate catches that a timing table cannot is an
optimisation that **moves** allocations rather than removing them.

The instrument is deliberately outside the workspace; §9.10 says why.
...
`MISMATCH-e16-header`: **the committed `results/E16-wallclock.md` predates the header and does not carry one.** The generator has stamped every document it produces since `5d37692` (cycle 7, T-04), and the file in `results/` was published before that, on a cycle-5 instance — so the table below is one whose provenance is recorded in this paragraph rather than in the artefact. The contract verdicts in it were also measured before T-02 changed what `reads`, `hits` and `misses` count, and before the `report` arm was measured with a second connection appending throughout. They are therefore read here as a direction and not as a current verdict; the marker clears when Host C republishes E16 under the new header, which is `run6.sh`'s section B.
...
Both sides are driven over the PostgreSQL wire protocol *through the same client*, over the same protocol path (recorded per CSV row), so neither is spared the protocol cost the other pays. **Every row is measured on both targets.** They were not: three of them read `NOT RUN` against reasons — "no write surface over the wire", "a scan-and-group-by surface is not exposed" — that were true when they were written and had stopped being true, so three quarters of this table reported a gap in the engine that was a gap in the harness's beliefs about it.

**Two rows say NOT MET, and that is the result.** The engine is at parity with PostgreSQL on durable OLTP against a contract of 5–10×, and under 2× on the analytical workload against a contract of 10–12×. Both are attributed in `docs/BENCHMARK.md`'s enumerated *Known limitations of the Nilestream path* — and for both, the attribution now ends in arithmetic showing the contract is not reachable in the shape it is written, which is a more useful thing to know than a factor:

* **OLTP.** A durable commit is an `fsync`, and one `fsync` per transaction caps a single writer at `1/fsync`: 4,450 ops/s on this device (E13). PostgreSQL measures around 2,700–3,100 here, so 5× is 15,000 and 10× is 30,000 — **both above the per-connection ceiling**, and no engine change reaches them. Group commit does: E13 measures 30,440 ops/s at batch 16, recovering 6.8× of the tax. But a batch of 16 needs sixteen *concurrent* writers, and this harness drives one synchronous connection, so the workload it measures can never form one. Reaching the contract needs a group-commit sequencer, a concurrent client and more than two cores together — and PostgreSQL gains from concurrency too, so the ratio afterwards is not predictable from these numbers.

  There is a second structural limit behind that, worth recording where the NOT MET is: **the daemon serves every query under one mutex.** `daemon.rs` holds an `Arc<Mutex<RevEngine>>` and takes it per query on a thread-per-connection model, so the engine is concurrent in its connections and serial in its work — which cannot use a second core for reads over an immutable base, and that is precisely the coordination-free read §7.5 claims. It does not affect the rows above, because they are measured on one connection; it is the first thing that would.
...
* **Analytical.** No longer a full fold: the keyed-aggregate fragment — `Source → (Filter | Map)* → Aggregate{sum, count}`, the same fragment the REV runtime accepts — is answered by one streamed pass over the ledger's posting records with a per-group accumulator and no intermediate Z-set. Statement by statement, the engine is **2.75× PostgreSQL** on `group by cur` and **1.18×** on `sum(amt) where amt < 0`, and 0.47× and 0.54× on the two that group by account and return ten thousand rows. The remaining gap is not the fold; it is producing and framing ten thousand rows, plus a protocol floor.

  That floor is where the contract stops being reachable. 10–12× PostgreSQL over this composite means 130–156 µs per statement *including* the round trip, and a round trip on this host measures about 120 µs — leaving 10–36 µs for everything the engine does over a base of twenty thousand postings. **The analytical contract as written is not reachable over a per-statement request/response protocol at this base size, whatever the engine does.** Restating it per statement, or over a pipelined workload, would make it reachable and would also help PostgreSQL; that is a decision for the specification, and it is recorded rather than quietly worked around.

  The row is also now a **common-set** ratio. It used to average five PostgreSQL statements against three Nilestream ones — PostgreSQL's set containing its cheapest statement and Nilestream's not, and one "pair" being `group by acct order by sum(amt) desc limit 10` against a plain `group by acct`, two different operations averaged as one. The statements are paired by operation; two that Nilestream cannot express are still measured on PostgreSQL, so their cost is on the record, and are excluded from the ratio.

What this table supports is therefore **an engine with a measured baseline, a characterized gap, and two contracts whose shape has been shown to put them out of reach on this architecture** — not a performance claim. §7's contract remains the target; two of its rows are unmet at this commit, against causes that are named and, where the cause is the contract's own shape, quantified.

The `report` row is the exception, and it is worth being precise about why it is not the analytical row in better clothes. It is not a faster fold. Both targets run the same statement over the same base, growing at the same rate under the same appender, and Nilestream's answer touches **no base row at all**: the server is asked (`explain`) before every run and the row is recorded `NOT RUN` rather than published if it answers anything but `report-from-view`. What the 2.68× measures is therefore the difference between reading a maintained answer and deriving one — which is the thesis's structural claim, appearing for the first time as a wall-clock number rather than as counted work.

The `point` row is an engine result rather than a protocol one. A single account's balance is answered by the REV runtime over the circuit the daemon's schema compiles to — partial materialisation under a budget, the absence lattice, an anchored upquery on a miss — so **the miss rate is a measurement**: at a residency budget of a quarter of the key space, with ninety percent of reads landing in the hottest one percent, the committed run's ten `point` rows report 0.14 to 0.16 (`results/E16-wallclock/point.csv`, column `miss_rate`). Parity with PostgreSQL's indexed aggregate while reconstructing one read in seven is a different claim from parity with a warm cache, and a stronger one. **The figure in this sentence was 0.30, and no row of the file it sits beside says that** — the generated document renders the column and this paragraph typed a number in, which is the failure mode the generator exists to prevent (cycle 9, F-73).

It was 1.00, and *by construction* rather than by measurement: the wire path evaluated the compiled circuit over a source scan and never consulted the view, while `read_stats` returned hits of zero and misses equal to reads. The column could not have reported anything else whatever the engine did, so the mechanism this chapter characterises was measured by nothing the daemon ran. The maintenance that makes the rate real is paid on the write path — about three allocations per transaction, and an `oltp` row that moved from 1.16× to 1.00× of PostgreSQL measured within one run — which is the maintain-versus-reconstruct trade of §9.3 appearing as a cost rather than as a diagram.

Memory is measured too, and separately, because a fold that is fast and a fold that is cheap are different claims: `results/E18-memory.md` reports allocations and bytes per operation for the ledger, the served path and the runtime, and `make memory` holds each to a budget. The figures a reader is most likely to want are that a served `group by cur` costs 25 allocations where it once cost 173,363, that a read of a hot key costs 20 and touches no base rows, and that the seeded ledger holds 415 bytes per posting.
...
E16 measures a ratio at one size, and a ratio at one size cannot tell a system that scales from one that happens to be quick at twenty thousand rows. The claim this thesis makes is asymptotic — H-F1 says a stream-first system's per-answer cost must not grow with accumulated input — so this experiment measures a *shape*.

Two axes, because they answer different questions, and three series, because Nilestream has two paths and reporting only the fast one would be publishing half the phase diagram: `postgres` recomputes, `nilestream:cold` folds the base, and `nilestream:warm` reads the maintained view. Every point in the warm series was confirmed by asking the server which path it would take *before* it was timed; a point that came back `fold` is `NOT RUN` rather than published under the warm name.

**Along the base**, with the answer held at 10,001 rows:

<!-- BEGIN:E23-base-medians results/E23-scaling.md#basemedians -->

*Generated from `results/E23-scaling.md`. Do not edit by hand.*
...
<!-- END:E23-base-medians -->

<!-- BEGIN:E23-base-slopes results/E23-scaling.md#baseslopes -->
...
A hundredfold increase in accumulated history — twenty thousand postings to two million — moves the maintained view's answer by a fraction of a millisecond, where both recomputing series grow by two orders of magnitude. **This is the measurement the architecture exists for**, and it is the only one in this chapter on which "matches PostgreSQL as the data grows" is a statement that could have come out false.

**The strict verdict is not stable, and that is reported rather than resolved by re-running.** Two full sweeps of the same binary on the same host read the warm slope as 1.18 × 10⁻⁷ ± 3.2 × 10⁻⁸ (positive: NOT MET) and as 5.4 × 10⁻⁸ ± 2.3 × 10⁻⁷ (flat: MET). Both are honest readings of their own nine points. They disagree because the quantity being asked about sits at the edge of what three sizes and three runs resolve — the smallest slope this design can distinguish from zero is about 4.5 × 10⁻⁷ ms per row of base, which is larger than the effect. Choosing whichever sweep gave the wanted verdict would be reporting the throw of a die as a property of the system, so the results file prints its own resolution floor beside the verdict and the ratio to the control, which is three orders of magnitude and stable across every sweep.

**Where the residual is.** Along this axis the answer has the same *number* of rows at every base size — the account count does not change — but not the same number of *bytes*: at a hundred times the base, each balance has summed a hundred times as many postings and needs about two more decimal digits. Ten thousand rows two bytes wider is a ten-percent larger answer. Plotting the same nine measurements against the answer's measured size rather than against the base size makes the warm series flat (9.1 × 10⁻³ ± 1.2 × 10⁻² ms per kilobyte, R² = 0.07) while both recomputing series stay positive. So what remains is the cost of serialising a bigger answer, which no design makes smaller, and not the cost of touching more state. This is offered as evidence about a cause; the contract row is still judged on the criterion as written.

**Along the answer**, with the base held fixed:

<!-- BEGIN:E23-output-medians results/E23-scaling.md#outputmedians -->
...
<!-- END:E23-output-medians -->

<!-- BEGIN:E23-output-slopes results/E23-scaling.md#outputslopes -->
...
<!-- END:E23-output-slopes -->

Here the claim is **parity**, deliberately. A report has to put its answer on the wire, and a system claiming to beat that would be claiming to send fewer bytes than the answer contains. The place where a maintained view can win is the first axis.

<!-- BEGIN:E23-asymptotic results/E23-scaling.md#asymptotic -->
...
**Why the error bars are the point.** A slope near zero always *looks* like the answer H-F1 wants, so a point estimate is not evidence: "flat" here means the confidence interval contains zero at two standard errors, and the fitting code's own tests include a case where a genuine thirty-fold growth must come out **positive**, so the criterion is not a rubber stamp for the hypothesis it tests.

Two things the harness refuses rather than reports, and both are repairs:

* **A slope from fewer than three distinct sizes.** With two the fit is exact, the residual degrees of freedom are zero, and the standard error is 0/0 — a number that would look like certainty and mean nothing.
* **A verdict whose control has no slope of its own.** The first run of the output axis held the base at 200,000 rows while the answer grew only from 201 to 1,001, so PostgreSQL's cost was dominated by an unchanging scan and its fitted output slope came out *negative* — which a comparison of point estimates read as parity being met. The verdict now reads `INCONCLUSIVE` and says to widen the range. A benchmark that can win on its baseline's measurement error can win on anything.

These two rows are what replaced the analytical contract's 10–12×, which was a geometric mean over ClickBench queries returning a handful of rows, carried into a workload returning ten thousand — where a fixed serialisation cost sits in both terms of the ratio and drags it toward 1.0× however fast the server is. The old wording is retained as `MISMATCH-A-01` and the cold composite stays in E16, because deleting either would hide half of what was learned.

### 9.14.6 What the compiler costs (E26)

`results/E26-compile-cost.md`, generated by `experiments e26`; `nilesc time FILE` produces the phase table.

H-S6 names three dependent variables and one of them — **compile time** — had no experiment for eight cycles. E14 measures the residual-violation half and §9.14.3 the checker's coverage; nothing had measured what the checking costs, which is the other half of "at no measurable runtime cost".

Two instruments, because neither alone is honest. **Deterministic counters** — tokens, items by class, IR operators, obligations accounted for — are a property of the program and the compiler, so a change to either moves a number and a change of machine does not. Those are what E26 commits. **Wall clock per phase** is host-shaped and is reported with its host: on the auditing container (2 cores, Xeon @ 2.10GHz), `examples/demo_bank.niles` — 158 lines, 16 items, 675 tokens — compiles in a median **243 µs**, distributed as typecheck 45%, parse 40% (of which lexing alone is 22%), lowering 7.5%, resolution 4.4%, and **the IR verifier 2.4%**.

The obvious third instrument is not available and the reason is worth recording: a whole-process instruction count under `callgrind`, which this project uses elsewhere precisely because instruction counts are deterministic, is **not** deterministic at this granularity — three runs of one binary over one file measured 2,402,561 / 2,410,322 / 2,409,454 I-refs, a 0.3% spread arising from process start-up and the environment rather than from compilation. A gate built on it would fire on the weather. Counters that the program itself produces do not have that problem, which is why E26 reports those.

**The verifier is not on the serving path, and neither is anything else in that table.** A statement is compiled once per distinct `(schema, statement text)` and served from a per-session cache thereafter — asserted, not assumed, by `the_front_end_runs_once_per_distinct_statement_and_not_per_execution`. That assertion is what makes H-S6's runtime-overhead claim a claim about the running system rather than about a compiler nobody runs on the hot path.

Measuring it found the cache's eviction policy to be the opposite of a cache. The limit is 256 plans per session and the policy was to **empty the cache** to admit the 257th, so a working set a little larger than the limit hit nothing: 1,000 distinct statements executed twice cost 2,000 compilations, where a cache exists to produce 1,000, and every one-off statement threw away a session's entire hot set. It is now first-in-first-out — one plan leaves to admit one plan — guarded by `one_new_statement_evicts_one_plan_and_not_the_whole_cache`. Neither policy survives a strict cyclic scan of `limit + 1` statements; that is Bélády's worst case and LRU shares it, and the test says so rather than claiming otherwise.
