...
**Premise risk on skew.** The literature disagrees about how skewed production access is, and the flat end of that range is unfavourable. Treated as a first-class design input: the optimizer exists precisely because the right materialization decision cannot be assumed, and H-S1's protocol reports the regimes separately rather than averaging them.
...
* *"The performance contract's four rows are targets the engine is on track for."* **Two are unmet, measured (§9.14.1).** Against PostgreSQL 16.13 on the same two-core machine, over the same wire protocol and the same protocol path, with `synchronous_commit = on` on one side and `SyncPolicy::Always` on the other: durable OLTP is **0.93×** against a contract of 5–10×, and the analytical workload is **0.13×** against 10–12×. Point lookups and durable commits are at parity, which two of the four rows ask for. The gaps are attributed to named items on `BENCHMARK.md`'s limitations list — per-query compilation on two cores, and a full fold behind an unkeyed scan — and not to the engine's correctness. What this establishes is an engine with a measured baseline and a characterized gap; the previous state of the table was three rows reading `NOT RUN` for reasons that had stopped being true.
* *"The served answer is the query the client sent."* **Refuted, and it is the most serious defect this review found.** The session compiled the client's SQL, resolved it, typechecked it, lowered it, verified the circuit — and discarded it: the answer came from a hard-coded fold of `sum(amt)` over an account scraped out of the query text with a digit scanner. Two different questions about one account returned the same number. §6.9's commitment that a wire protocol is *a surface, not a semantics* was false of the artifact that argued for it, in the purest form of the failure it names: one path that parses and another that answers.
* *"The type checker implements λ_niles."* **Refuted in every clause that was checked.** `let x: Money<usd> = "hello" + true;`, `let auth: Auth<authorize<usd>> = 42;`, and a `ledger_consistent` view reading a `bounded` view through a helper all passed `nilesc check` with zero errors; a `txn` whose callee posted one half of a transfer was neither proved, nor warned about, nor discharged to the runtime — it was not counted. All are refused now, each by a named mutant, and Theorem 4.4's clause (3) is narrowed by MISMATCH-T-11-overdraw because the static balance bound it asks for cannot be built on this AST.
* *"The SQL surface lowers what it claims to."* **Refuted on ten forms.** `SELECT a, b` emitted no projection, `DISTINCT` and the set operators were parsed and never read, a two-table `FROM` used the first table, `JOIN … ON` resolved its condition against the left schema only — so it found nothing and the join returned the cross product, silently, with a circuit that verified — and `where x is null` did not parse at all. The mapping table called them `Lowered`. §4.7(c) and §9.7's "golden-file α-equivalence tests" described a file that did not exist.


**Still standing, with the result that would overturn each.**

* A conservation violation traceable to the *model* rather than to an implementation defect would falsify C1 and halt the programme (K2). Five seeds × 10,000 transfers under continuous eviction have not produced one (§9.2.1); that is corroboration, not proof, and a single counterexample would still end it.
* A measured crossover far outside Theorem 4.2's band, in either direction, would mean the cost model omits a first-order term. *Except that no band is derivable*: clause (ii)'s Θ(Z) term carries no constant, so the theorem asserts that a crossover **exists** and says nothing about where. The prototype located one between memory prices 0.0005 and 0.002 in counted-work units (§9.3.3), which the theorem is consistent with rather than confirmed by. C2 is therefore an existence claim, refuted by finding no crossover anywhere in a swept price range and not by finding one at an unexpected price. `results/E16-band.md`, committed before the durable run, registers the one side that *is* derivable and names the missing constant. See MISMATCH-F-08.
* **Two Part 0 rows are unmet at this commit, and a third result would change the reading of them.** If per-query compilation is removed (a plan cache on the simple path) and an unkeyed scan stops materialising the whole base, and the OLTP and analytical rows *still* sit near 1×, then the gap is the engine's rather than the harness's and §7's contract is the thing that has to move. Until those two limitations are lifted, the honest report is a measured baseline and a characterized gap — which is what §9.14.1 now says.
* A fixed policy beating the adaptive optimizer across the workload suite would reduce C5 to a planning-time heuristic. The measured margin of cost-aware eviction over LRU is real but uneven — 28.6% on reconstruction work, 2.8% on aggregate delay (§9.4.2, generated from `results/e6_policies.csv`; the 31% and 4.1% an earlier draft carried were not the medians of that file) (§9.4.2) — so this one is closer to the edge than the others.
* A banking product from Section 6.22 that cannot be expressed without a kernel change would falsify the "banking as a library" claim and, with it, part of the generality thesis. **The falsifier has now been looked for across the whole scope and not found**: all twenty-nine product lines of the coverage matrix are implemented, as compositions of the seven mechanisms, and none required a change to the kernel. That is corroboration and not proof — twenty-nine products chosen by the author is a sample the author drew — but it is a considerably stronger corroboration than the sixteen rows the previous revision reported, because the thirteen added are the exotic end: clearing and prime brokerage, structured financing, distressed portfolios, trust and fiduciary structures.

  The closest call is worth recording, because a near miss is where a refutation condition earns its keep. A distribution **waterfall** — a preferred tier paid in full before a residual tier sees anything — looks like a new primitive, and an eighth mechanism was drafted for it. It is not one: a waterfall is the fractional-participation mechanism applied to a *sequence of remainders*, and the subtraction and minimum between allocations touch no ledger and produce no postings, so by §6.22's own definition they are arithmetic rather than mechanism. The guardrail for admitting an M8 — that it be expressible as a pattern of the balanced posting set, *and* that at least three product lines require it — failed on both halves, since a waterfall is a pattern of M4 and is required by two. The seven stand.

  One further row is worth naming because it tests the claim from the other side. **Risk analytics** carries no M1 at all: it reads and prices and books nothing. The obvious test of "products reach the ledger only through the kernel" is to write products that post and check they cannot get round it, and a system can pass that test while still growing a hidden write path — a risk report that "adjusts" a position, a valuation that writes back a mark. Those are the ones that corrupt a ledger quietly, because nobody audits a report. A read-only product is where such a path would appear, and a test scans that module's own source and fails if the posting machinery is reachable from it.
...
What Nilestream demonstrably provides beyond the PostgreSQL construction, on evidence in this repository, is: per-view consistency rungs with a rung-monotonicity check (a view served at ℓ may not read a view served below ℓ), reconstruction paths that are *verified* to be anchored rather than assumed to be, counted work exposed as a first-class measurement, and an IR verifier in the trusted base so the compiler need not be. Each is cumulative. None is enabling.

This list used to include *"cross-shard commit whose coordinator is itself a ledger group"*, and that entry is withdrawn. `cross_shard.rs` implements the protocol's **decision logic** — decide, persist, then tell anyone, with the ordering enforced and tested — over an in-memory participant set; `persist_decision` sets a boolean. There is no ledger group behind it, no segment, no recovery of a coordinator that has forgotten its decision, and no network. The design commitment that a coordinator should be a ledger group is stated in the module's own documentation and is a *design*, not an artefact, and a list of what a repository demonstrably provides may not carry a design. The protocol logic remains a real contribution at the grade it is: a simulated state machine with its safety property checked. The correct sentence for the abstract is that **the theory is the contribution and Nilestream is its instrument**, which is what §11.4 already says and what the rest of the document must be brought into line with.
...
**The engine surface: compatibility is sufficient and replacement is unnecessary.** §11.5.2 settles this. Wire compatibility is the right delivery vehicle, and this repository takes it seriously enough to serve one protocol and to be exact about the difference between the one it serves and the two it does not:

* **PostgreSQL wire protocol v3 — served.** The simple query path and the extended path (`Parse`/`Bind`/`Describe`/`Execute`/`Sync`, with plans keyed by schema epoch) are both wired into the connection loop, and both are driven by `psql` — a real libpq client, not an in-repo one written against the same understanding of the protocol as the server — in `nilestream-server/tests/psql_conformance.rs`. Reads, `INSERT`s and multi-statement transactions all cross it.
* **MySQL packet framing — a codec with no listener.** Frames encode and decode, with tests; nothing calls them. §7.3 specifies the protocol and this repository does not serve it. An earlier draft of this paragraph listed it beside PostgreSQL as though the two were in the same state.
* **TLS — a negotiation state machine with no provider.** `tls.rs` implements the `SSLRequest`/`N` exchange and the boot-time policy check for both protocols, and the provider compiled in is `NoProvider`, which refuses every accept. The daemon runs `TlsConfig::insecure()`. What exists is the *state machine* and the refusal discipline around it; what does not exist is transport security.

The design commitment underneath is unchanged and is the part that matters: a wire protocol is a *surface, not a semantics*. There is no compatibility layer with its own execution path, because two ways to compute an answer is two answers that can disagree — and until the revision that produced this paragraph, that commitment was false of the served protocol too: the session compiled the client's SQL, verified the circuit, and then discarded it in favour of a hard-coded fold. Two different questions about one account returned the same number.
...
| Extended query protocol refused with an open design question | `extended.rs`; epoch-keyed plan cache, 11 tests, **and wired into the connection loop** — it was reachable from no listener when this row was first written, so the row described a module rather than a capability | §7.4, §11.5.4 |
| No distributed read path | `distributed.rs`, 10 tests. **A model, not a path**: nothing in the server or the experiments calls it, and no read in this repository is served across a shard boundary | §7.4 |
| No cross-shard commit | `cross_shard.rs`, 12 tests. **The decision logic, simulated**: `persist_decision` sets a boolean, the participant set is in memory, and there is no ledger group, no segment and no network | §8.6, §11.5.3 |
| No MySQL wire | `mysql_wire.rs`, 11 tests. **A codec with no listener**: frames encode and decode and nothing calls them | §7.3, §11.5.4 |
| No TLS boundary | `tls.rs`, 17 tests. **A negotiation state machine with no provider**: `NoProvider` refuses every accept and the daemon runs `TlsConfig::insecure()` | §6.12, §11.5.4 |
...
| **`select k from t where t.z = 1` returned every row** | `sql_depth` in the parser; `Lx::predicate` replaces the silent `LitBool(true)` — **on the SQL surface only.** The pipeline surface kept `unwrap_or(Scalar::LitBool(true))` for another eleven months, so `postings.where(\|p\| p.nonexistent == 1)` still returned every posting, and this row claimed the site was pinned. Both are pinned now, with nine further `unwrap_or` sites in the same lowering | §9.14.2 |
...
**A pattern worth naming, since it has now recurred three times.** `Err(_) => 0` in the kernel, `sum` over an empty group, and `unwrap_or(LitBool(true))` in the lowering of a `where` clause are the same defect in three costumes: an absence given a *reasonable default* that is a wrong answer wearing a plausible shape. In each case the failure mode is not a crash and not an obviously wrong number, but a well-formed answer that no answer-level test can distinguish from the right one — a balance that reads zero, a total that reads zero, a view that returns every row. §3.3's lattice of absence exists because absences are not interchangeable and not substitutable by a value; these three are what happens when that discipline is not carried down into the implementation. The rule the repository now follows is that an absence gets a *named* representation or a diagnostic, never a default, and the sites are pinned by tests that assert the default is gone rather than merely that the current behaviour is right.

**It has now recurred fourteen times, and the count is the point.** A review that went looking for the pattern rather than waiting for it found eleven more instances of the same shape, every one of them in code written after the paragraph above: a `where` predicate defaulting to `true` on the *pipeline* surface while this thesis said the site was pinned; an aggregate's value defaulting to column 0, so a `sum` returned an account id; a join's right key defaulting to the left's, so two relations were joined on column *position*; a `limit` the lowering could not read becoming `u64::MAX`, which returns every row; an unrecognised stage lowering to the identity; a money literal in an undeclared currency taking the first declared currency's index; a fixpoint's measure defaulting to a constant, which is a termination argument that proves nothing; `valid_at` discarding its date and pinning to nothing at all; `is_conservation_transparent` answering `true` through a `_ =>` arm for four operators including `Map` and `avg`; `derive_key` losing the key through the same wildcard for three operators that cannot change one; and a served answer that ignored the query entirely. The rule is not "fix the three sites". It is that **a wildcard arm in a soundness predicate and an `unwrap_or` at a lowering site are the same defect**, and both are now compile errors in the operator set and diagnostics in the lowering.
