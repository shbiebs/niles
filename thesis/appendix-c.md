...
**Deployment.** A single static binary per target. Configuration is declarative (contract defaults, τ, memory budgets — Appendix D.6; the storage tiers named there are specification and nothing implements them). State directories separate by durability class: the base segments (the only truth), derived view state (disposable), and keys (deployment-profile).
