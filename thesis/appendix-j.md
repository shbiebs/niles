...
**Position B.** *Withdrawn in part, and recorded as it was held:* six — the four above, plus an adaptive materialization calculus with competitive guarantees, plus a generality result replacing the unprovable "replaces SQL" with provable statements.

**Verdict: B, extended to seven by experiment.** C1–C4 stand exactly as stated in Position A. Two are added because they answer questions C1–C4 raise but do not close: C2 says *where* partiality stops paying, but nothing says *what the runtime should therefore do* — that is C5, the mode calculus over {absent, demand, full, spilled, tiered}, and §9.3.4's measured interior optimum is direct evidence that a fixed policy is wrong. *Position B's wording is recorded as it was held:* the "competitive guarantees" in it are **withdrawn** — the optimizer is specified and not built, and no competitive ratio is claimed or measured anywhere in this thesis (Section 4.6, H-S7). And the thesis's own C6 asserts SQL-completeness, which is not a well-formed claim without a definition — that is C6.
